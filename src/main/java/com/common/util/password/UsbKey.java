...
public class UsbKey {
	 ...
	 * @return
	 */
	public static boolean digestComp(String clientDigest, String randomData,
		...
		for (int i = 0; i < 64; i++) {
			if (i < keyLen) {
				iResult[i] = (byte) ((iPad.charAt(i) & 0x00FF) ^ (passWord
						.charAt(i) & 0x00FF));
			} else {
		...
		for (int i = 0; i < 64; i++) {
			if (i < keyLen) {
				oResult[i] = (byte) ((oPad.charAt(i) & 0x00FF) ^ (passWord
						.charAt(i) & 0x00FF));
			} else {
